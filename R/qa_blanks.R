#' QA Blanks
#'
#' Quality assurance data - blanks samples. Blanks are unexposed sample collection devices (e.g., filters) that are transported with the exposed sample devices to assess if contamination is occurring during the transport or handling of the samples.
#'
...
#'@rdname qa_blanks
#'@export
qa_blanks_by_site <- function(param, bdate, edate, state, county, site) {
	getAQS("qaBlanks/bySite", param = param, bdate = bdate, edate = edate, state = state, county = county, site = site, auth = TRUE)
}

#'@rdname qa_blanks
#'@export
qa_blanks_by_county <- function(param, bdate, edate, state, county) {
	getAQS("qaBlanks/byCounty", param = param, bdate = bdate, edate = edate, state = state, county = county, auth = TRUE)
}

#'@rdname qa_blanks
#'@export
qa_blanks_by_state <- function(param, bdate, edate, state) {
	getAQS("qaBlanks/byState", param = param, bdate = bdate, edate = edate, state = state, auth = TRUE)
}

#'@rdname qa_blanks
#'@export
qa_blanks_by_pqao <- function(param, bdate, edate, pqao) {
	getAQS("qaBlanks/byPQAO", param = param, bdate = bdate, edate = edate, pqao = pqao, auth = TRUE)
}

#'@rdname qa_blanks
#'@export
qa_blanks_by_ma <- function(param, bdate, edate, ma) {
	getAQS("qaBlanks/byMA", param = param, bdate = bdate, edate = edate, ma = ma, auth = TRUE)
}
