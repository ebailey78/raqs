#' QA Flow Rate Audits
#'
#' Quality assurance data - flow rate audits. At least twice year, each PM monitor must have its flow rate measurement audited by an expert using a different method than is used for flow rate verifications.
#'
...
#'
#'@name qa_flow_rate_audits
NULL

#'@rdname qa_flow_rate_audits
#'@export
qa_flow_rate_audits_by_site <- function(param, bdate, edate, state, county, site) {
	getAQS("qaFlowRateAudits/bySite", param = param, bdate = bdate, edate = edate, state = state, county = county, site = site, auth = TRUE)
}

#'@rdname qa_flow_rate_audits
#'@export
qa_flow_rate_audits_by_county <- function(param, bdate, edate, state, county) {
	getAQS("qaFlowRateAudits/byCounty", param = param, bdate = bdate, edate = edate, state = state, county = county, auth = TRUE)
}

#'@rdname qa_flow_rate_audits
#'@export
qa_flow_rate_audits_by_state <- function(param, bdate, edate, state) {
	getAQS("qaFlowRateAudits/byState", param = param, bdate = bdate, edate = edate, state = state, auth = TRUE)
}

#'@rdname qa_flow_rate_audits
#'@export
qa_flow_rate_audits_by_pqao <- function(param, bdate, edate, pqao) {
	getAQS("qaFlowRateAudits/byPQAO", param = param, bdate = bdate, edate = edate, pqao = pqao, auth = TRUE)
}

#'@rdname qa_flow_rate_audits
#'@export
qa_flow_rate_audits_by_ma <- function(param, bdate, edate, ma) {
	getAQS("qaFlowRateAudits/byMA", param = param, bdate = bdate, edate = edate, ma = ma, auth = TRUE)
}
