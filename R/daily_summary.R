#' Daily Summary
#'
#' Returns data summarized at the daily level. All daily summaries are calculated on midnight to midnight basis in local time. Variables returned include date, mean value, maximum value, etc.
#'
...
#'@rdname daily_summary
#'@export
daily_data_by_site <- function(param, bdate, edate, state, county, site, cbdate, cedate) {
	getAQS("dailyData/bySite", param = param, bdate = bdate, edate = edate, state = state, county = county, site = site, cbdate = cbdate, cedate = cedate, auth = TRUE)
}

#'@rdname daily_summary
#'@export
daily_data_by_county <- function(param, bdate, edate, state, county, cbdate, cedate) {
	getAQS("dailyData/byCounty", param = param, bdate = bdate, edate = edate, state = state, county = county, cbdate = cbdate, cedate = cedate, auth = TRUE)
}

#'@rdname daily_summary
#'@export
daily_data_by_state <- function(param, bdate, edate, state, cbdate, cedate) {
	getAQS("dailyData/byState", param = param, bdate = bdate, edate = edate, state = state, cbdate = cbdate, cedate = cedate, auth = TRUE)
}

#'@rdname daily_summary
#'@export
daily_data_by_box <- function(param, bdate, edate, minlat, maxlat, minlon, maxlon, cbdate, cedate) {
	getAQS("dailyData/byBox", param = param, bdate = bdate, edate = edate, minlat = minlat, maxlat = maxlat, minlon = minlon, maxlon = maxlon, cbdate = cbdate, cedate = cedate, auth = TRUE)
}

#'@rdname daily_summary
#'@export
daily_data_by_cbsa <- function(param, bdate, edate, cbsa, cbdate, cedate) {
	getAQS("dailyData/byCBSA", param = param, bdate = bdate, edate = edate, cbsa = cbsa, cbdate = cbdate, cedate = cedate, auth = TRUE)
}
